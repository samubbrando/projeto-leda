...
def measure_insertion(data: List[int]) -> float:
    """Mede o tempo para inserir todos os elementos de uma amostra em uma LinkedList vazia."""
    start_time = time.perf_counter()
    
    ll = LinkedList()
    for item in data:
        ll.addLast(item) # Usando addLast. inserção O(1) no final
        
    end_time = time.perf_counter()
    return (end_time - start_time) * 1000 # Retorna em milissegundos

def measure_search(data: List[int], populated_list: LinkedList) -> float:
    """Mede o tempo para buscar todos os elementos de uma amostra em uma LinkedList pré-populada."""
    start_time = time.perf_counter()
    
    for item in data:
        populated_list.getByValue(item)
        
    end_time = time.perf_counter()
    return (end_time - start_time) * 1000

def measure_deletion(data: List[int], populated_list: LinkedList) -> float:
    ...
    for item in data:
        list_to_delete.removeByValue(item)
...
def main():
    ...
    print("Iniciando medição de performance da LinkedList...")
    ...
    for filename in SAMPLE_FILES:
        ...
        for line in lines:
            ...
            populated_linkedlist = LinkedList()
            for item in data:
                populated_linkedlist.addLast(item)
    ...
    if all_results:
        csv_path = os.path.join(MEASUREMENT_PATH, "linkedlist_results.csv")
