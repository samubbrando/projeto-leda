...
def measure_deletion(data: List[int], populated_list: SkipList) -> float:
    ...
    list_to_delete = SkipList()
    for key, value in populated_list.items():
        list_to_delete.insert(key, value)
    
    start_time = time.perf_counter()
    
    for item in data:
        list_to_delete.pop(item)
...
def main():
    ...
    print("Iniciando medição de performance da SkipList (pyskiplist)...")
    ...
    if all_results:
        csv_path = os.path.join(MEASUREMENT_PATH, "pyskiplist_results.csv")
